...
public final class CreateSimpleWorking {
    ...
    public PDDocument create(ShopDetails details , Bill bill) throws IOException {
        ...
        try  {
            ...
            contentStream.addLine(0, 520, 400, 520);
            contentStream.addLine(0, 470, 400, 470);
            createTable(document, page, contentStream,bill);

            document.save("/Users/arjun.n/Desktop/BillingSoftware/invoice/"
                    +bill.getType()+"-"+details.getName()+"-"+bill.getInvoiceId()+".pdf");
            return document;
        }catch(Exception e){
           return document;
        }
    ...
    public void createBillHeader(PDDocument document, PDPage page, ShopDetails shopDetails, Bill bill )throws IOException {
        ...
        billHeader(document, page, 140, 550, 200, 20, false, shopDetails.getName(),12);
        billHeader(document, page, 140, 535, 200, 20, false, shopDetails.getAddress(),12);
        billHeader(document, page, 140, 520, 200, 20, false, shopDetails.getLocality(),12);
            
        billHeader(document, page, 0, 490, 200, 30, false, "GST:" +shopDetails.getGst_no(),10);
        billHeader(document, page, 300, 490, 100, 30, false, "BillNo: " +bill.getInvoiceId(),10);
        billHeader(document, page, 300, 470, 100, 30, false, "DATE:" +bill.getDay()+"/"+bill.getMonth()+"/"+bill.getYear(),10);
       
        
        billHeader(document, page, 270, 170, 250, 30, false, "Total CGST :" +details.roundOff(bill.getTotalCgst()),10);
        billHeader(document, page, 270, 150, 250, 30, false, "Total SGST :" +details.roundOff(bill.getTotalSgst()),10);
        billHeader(document, page, 270, 130, 250, 30, false, "Total Amount :" +bill.getTotalAmt(),10);
    }
    
    public void billHeader(PDDocument document ,PDPage page, int x, int y, int weight, 
            int height, boolean format, String message, int fontSize)throws IOException{
        PDTextField nameOfCompany = createTextBox(document,fontSize);
    ...
    public void createTable(PDDocument mainDocument, PDPage myPage, PDPageContentStream contentStream, Bill bill) throws IOException {
       ...
       float tableWidth = myPage.getMediaBox().getWidth();
        ...
        BaseTable table = new BaseTable(430, yStartNewPage, bottomMargin, tableWidth, margin, mainDocument, myPage, true, drawContent);
    ...
    public PDTextField createTextBox(PDDocument document, int fontSize) {
        ...
        defaultAppearanceString = "/Helv "+size+" Tf 0 0 0 rg";
